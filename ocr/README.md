...
| [`hpd-parsing-server.py`](https://huggingface.co/datasets/uv-scripts/ocr/blob/main/hpd-parsing-server.py) | [HPD-Parsing](https://huggingface.co/PaddlePaddle/HPD-Parsing) | 1B | vLLM server (vendor fork) | **Layout + text in one pass** — adds an `hpd_blocks` column (type, bbox, reading order; bbox **0–1000 normalized**). Hierarchical parallel decoding: 94.91 OmniDocBench v1.6 at 4,752 TPS. Server-only by design; needs Paddle's own image, which has **no `uv`** — see the script docstring for the `hf jobs run` bootstrap. Apache 2.0 |
