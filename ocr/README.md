...
| [`falcon-ocr-1.5-vendor.py`](https://huggingface.co/datasets/uv-scripts/ocr/blob/main/falcon-ocr-1.5-vendor.py) | [Falcon-OCR 1.5](https://huggingface.co/tiiuae/Falcon-OCR) | 0.3B | TII image (vLLM fork + layout pipeline) | The vendor's deployable product: boots `ghcr.io/tiiuae/falcon-ocr` (digest-pinned, baked weights hash-checked) and drives its `/falconocr/parse` API. `e2e` default per the card, `--layout` for the layout+OCR pipeline. Image has no `uv` → launch via `hf jobs run … -- bash -lc 'pip install uv && uv run URL'` |
