...
| `unlimited-ocr-vllm.py` | [Unlimited-OCR](https://huggingface.co/baidu/Unlimited-OCR) | 3.3B | vLLM | DeepSeek-OCR-based; layout-grounded markdown (`--strip-grounding` for clean text). Single-image only — needs Baidu's **dedicated `vllm/vllm-openai:unlimited-ocr` image** (`-cu129` on Hopper). For multi-page "long-horizon" parsing use **SGLang** (validated working; vLLM multi-page garbles — [serving doc](serving-unlimited-ocr.md)). MIT |
