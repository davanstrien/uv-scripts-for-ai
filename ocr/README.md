...
| [`falcon-ocr.py`](https://huggingface.co/datasets/uv-scripts/ocr/blob/main/falcon-ocr.py) | [Falcon-OCR v1](https://huggingface.co/tiiuae/Falcon-OCR/tree/42ec56b72a23984ac059e7c8a6d397a8529423fe) | 0.3B | falcon-perception | Smallest VLM in collection. #1 on multi-column docs and tables (olmOCR), Apache 2.0. Pinned to the last v1 commit (repo root became v1.5 in-place on 2026-09-11); `--revision` overrides. Carries a `[tool.hf-jobs]` header (flavor + HF_TOKEN) |
| [`falcon-ocr-1.5.py`](https://huggingface.co/datasets/uv-scripts/ocr/blob/main/falcon-ocr-1.5.py) | [Falcon-OCR 1.5](https://huggingface.co/tiiuae/Falcon-OCR) | 0.3B | falcon-perception | Same recipe and engine, pinned to the v1.5 release head (`fe757d59…`). On BHL scans: better content CER and recall than v1, cleaner blank pages, but occasionally hallucinates long text on plates |
| [`falcon-ocr-1.5-vendor.py`](https://huggingface.co/datasets/uv-scripts/ocr/blob/main/falcon-ocr-1.5-vendor.py) | [Falcon-OCR 1.5](https://huggingface.co/tiiuae/Falcon-OCR) | 0.3B | TII image (vLLM fork + layout pipeline) · saturate driver | The vendor's deployable product: boots `ghcr.io/tiiuae/falcon-ocr` (digest-pinned, baked weights hash-checked) and pumps `/falconocr/parse` through `saturate` (adaptive window capped at 8, resumable parquet output, error rows). `e2e` default per the card, `--layout` for newspapers/huge pages. Image has no `uv` → launch via `hf jobs run … -- bash -lc 'pip install uv && uv run URL'` |
